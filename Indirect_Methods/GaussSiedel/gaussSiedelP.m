...
for times = 1:MAX_LIMIT_ITERATIONS
    ...
    for i  = 1:length(A)
        ...
        for j = 1:length(A)
            if(i~=j)
                multiplier = tempValues(j,times);
                % New Code added, different from Jacobi Method
                % Just add below if case, if you want to use Gauss Siedel
                % or remove if you want to use Jacobi Method
                if(j < i)
                    
                    multiplier = tempValues(j,times+1);
                    
                end
                tsum = tsum + A(i,j)*multiplier;
            end
        end
